...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(
            "SELECT new com.storder.order.order.dto.user.OrderSummaryDto("
                    + "o.orderId, "
                    + "o.createdAt, "
                    + "s.storeImage, "
                    + "s.storeName, "
                    + "SUM(om.menuTotalPrice), "
                    + // 가게별 총 주문 금액
                    "o.endAt) "
                    + // 주문 완료 일시
                    "FROM Order o "
                    ...
                    + "JOIN m.store s "
                    + "WHERE o.user.userId = :userId "
                    + "AND o.orderStatus = 'READY' "
                    + // 주문 상태가 READY인 것만 필터링
                    "AND o.createdAt BETWEEN :startDate AND :endDate "
                    + "GROUP BY o.orderId, s.storeId, o.createdAt, s.storeImage, s.storeName, o.endAt "
                    + // 주문 ID 기준 그룹화
