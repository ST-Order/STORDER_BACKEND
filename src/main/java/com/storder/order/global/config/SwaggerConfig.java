package com.storder.order.global.config;
...
public class SwaggerConfig {
    ...
    public OpenAPI openAPI() {
        Info info =
                new Info()
                        .title("ST:ORDER API Document")
                        .version("1.0")
                        .description("환영합니다! API 문서는 ST:ORDER의 API를 사용하는 방법을 설명합니다.\n")
                        .contact(
                                new io.swagger.v3.oas.models.info.Contact()
                                        .email("storder82@gmail.com"));
    ...
    /**
     * Swagger 문서의 각 API 동작(Operation)을 사용자 정의하기 위해 사용 ApiErrorCodeExample 어노테이션을 찾습니다. 이 어노테이션은 특정
     * 메서드에 정의된 에러 코드 예시를 설정하는 데 사용됩니다.
     */
    @Bean
    public OperationCustomizer customize() {
        return (Operation operation, HandlerMethod handlerMethod) -> {
            ...
            if (!tags.isEmpty()) operation.setTags(Collections.singletonList(tags.get(0)));
     ...
     * GlobalCodeException 타입을 가지며, GlobalCodeException 의 errorReason 와,ExplainError 의 설명을 문서화시킵니다.
     */
