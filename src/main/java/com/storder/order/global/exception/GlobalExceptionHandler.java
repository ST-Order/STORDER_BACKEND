...
import com.storder.order.global.dto.ErrorResponse;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleExceptionInternal(
        ...
        ErrorResponse errorResponse =
                new ErrorResponse(
                        httpStatus.value(), httpStatus.getReasonPhrase(), ex.getMessage(), url);

        return super.handleExceptionInternal(ex, errorResponse, headers, status, request);
    }

    // 주로 요청 본문이 유효성 검사를 통과하지 못할 때 발생합니다 (예: @Valid 어노테이션 사용 시) MethodArgumentNotValidException 예외를
    // 처리하는 메서드
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(
        ...
        ErrorResponse errorResponse =
                new ErrorResponse(
                        httpStatus.value(), httpStatus.getReasonPhrase(), errorsToJsonString, url);

        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
    }

    /**
     * Request Param Validation 예외 처리 유효성 검사 제약 조건이 위반되었을 때 발생합니다. (예: @NotNull, @Size, @Email 어노테이션
     * 사용 시)
     */
    @ExceptionHandler(ConstraintViolationException.class)
    public ResponseEntity<ErrorResponse> constraintViolationExceptionHandler(
            ConstraintViolationException ex, HttpServletRequest request) {
        Map<String, Object> bindingErrors =
                new HashMap<>(); // 유효성 검사 실패 필드와 해당 오류 메시지를 저장하기 위한 맵을 생성
                            ...
                            List<String> propertyPath =
                                    List.of(
                                            constraintViolation
                                                    ...
                                                    .toString()
                                                    .split("\\."));
                            // 마지막 요소를 추출하여 실제 필드 이름을 가져옵니다. 예를 들어, 경로가 user.address.street라면
                            // street가 추출됩니다.
                            String path =
                                    propertyPath.stream()
