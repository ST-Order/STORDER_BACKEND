...
public class StoreNoticeController {
	...
	public ResponseEntity<ApiResponse<List<SellerNotificationResponseDto.Brief>>> getAllNotifications() {
	...
	public ResponseEntity<ApiResponse<SellerNotificationResponseDto.Detail>> getNotificationDetails(@PathVariable("notificationId") String notificationId) {
		return ResponseEntity.ok(ApiResponse.success("공지사항 상세 조회에 성공하였습니다.", null));
	}

	@PostMapping
	@Operation(summary = "공지사항 등록", description = "공지사항을 등록합니다.")
	public ResponseEntity<ApiResponse<String>> postNotification(@RequestBody SellerNotificationRequestDto.Creation request) {
	...
	public ResponseEntity<ApiResponse<String>> modifyNotification(@RequestBody SellerNotificationRequestDto.Creation request) {
